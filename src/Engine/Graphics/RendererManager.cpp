...
namespace cxc {
	...
	bool BaseRender::CreateShaderProgram()
	{
		...
		if (!CompileShader(VertexShader, VertexShaderCode, OutResultLog)) {
			std::cout << "VS compile failed, Path : " << VertexShaderPath << std::endl;
			std::cout << OutResultLog << std::endl;
			return false;
		}

		if (!CompileShader(FragmentShader, FragmentShaderCode, OutResultLog)) {
	...
	bool BaseRender::CompileShader(GLuint ShaderID, const std::string &SourceCode, std::string& OutResultInfo) const
	{
	...
	void BaseRender::CreateLightInfo(const glm::vec3 &pos, const glm::vec3 &dir, eLightType type, eInteractionType interactive)
	{
	...
	void BaseRender::SetLightType(eLightType type) noexcept
	{
		if (pLightInfo)
	...
	eLightType BaseRender::GetLightType() const noexcept
	{
		if (pLightInfo)
